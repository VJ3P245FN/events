...
class Rest
{
	...
	function __construct($init=false)
	{
		if( $init )
		{
			...
			// read badges of a user
			\register_rest_route(
				self::getNamespace(),
				'/badges/(?P<email>[^\@\s\/]+\@[^\@\s\/]+)',
				array(
					'methods'  => 'GET',
					'callback' => array($this, 'getUserBadges'),
					'permission_callback' => array($this, 'permissionRead'),
					'args'     => array(
						'email' => array(
							'required' => true,
			...
			// read users owning the badge
			\register_rest_route(
				self::getNamespace(),
				'/badge_owners/(?P<badge_id>[0-9]+)',
				array(
					'methods'  => 'GET',
					'callback' => array($this, 'getBadgeUsers'),
					'permission_callback' => array($this, 'permissionRead'),
					'args'     => array(
						'badge_id' => array(
							'required' => true,
			...
			\register_rest_route(
				self::getNamespace(),
				'/badge_rarity/(?P<badge_id>[0-9]+)',
				array(
					'methods'  => 'GET',
					'callback' => array($this, 'getBadgeRarity'),
					'permission_callback' => array($this, 'permissionGeneral'),
					'args'     => array(
						'badge_id' => array(
							'required' => true,
	...
	function getUserBadges($data)
	{
		...
		return $wpdb->get_results($wpdb->prepare(
			"SELECT badge_id, assign_date FROM {$wpdb->lwsWooRewardsBadges} WHERE user_id=%d",
			$user->ID
		));
	}

	function getBadgeUsers($data)
	{
		if (!$this->currentUserCan('lws_wr_read_other_points')) {
			return new \WP_Error('rest_forbidden', __('Cannot view others data', 'woorewards-pro'), array('status' => 403));
		}
		$badge = new \LWS\WOOREWARDS\PRO\Core\Badge($data['badge_id'], true);
		if (!$badge->isValid()) {
			return new \WP_Error('no_badge', __('Unknown Badge', 'woorewards-pro'), array('status' => 404));
		}

		global $wpdb;
		return $wpdb->get_col($wpdb->prepare(
			"SELECT user_id FROM {$wpdb->lwsWooRewardsBadges} WHERE badge_id=%d",
			$badge->getId()
		));
	}

	function getBadgeRarity($data)
	{
		$badge = new \LWS\WOOREWARDS\PRO\Core\Badge($data['badge_id'], true);
		if (!$badge->isValid()) {
			return new \WP_Error('no_badge', __('Unknown Badge', 'woorewards-pro'), array('status' => 404));
		}
		return $badge->getBadgeRarity();
	}
